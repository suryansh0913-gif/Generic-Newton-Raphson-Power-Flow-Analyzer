...
def load_from_raw(raw_file):
        ...
        for line in f:
            ...
            if section == "BUS" and "'" in line:
                ...
                try:
                    ...
                    Pd = float(parts[5]) / 100
                    Qd = float(parts[6]) / 100
                ...
                try:
                    ...
                    Pg = float(parts[2]) / 100
                    Qg = float(parts[3]) / 100
    ...
    for b in buses:
        ...
        b["P_sch"] = b["P_gen"] - b["P_load"]

        b["Q_sch"] = b["Q_gen"] - b["Q_load"]
...
def print_results(buses, V, Y, bus_idx, method):
    ...
    print("\nBUS TYPES CHECK")
    for bus in buses[:20]:
        print("Bus =", bus["id"], "Type =", bus["type"])
...
def calculate_losses(lines, V, bus_idx):
    ...
    for line in lines:
        ...
        P_loss = np.real(Sij + Sji)
        Q_loss = np.imag(Sij + Sji)
        ...
        print(
            f"{line['from']}->{line['to']} : "
            f"P_loss={P_loss:.6f} pu , "
            f"Q_loss={Q_loss:.6f} pu"
        )

    print("\nTOTAL SYSTEM LOSSES")
    print(f"Active Loss   = {total_P_loss:.6f} pu")
    print(f"Reactive Loss = {total_Q_loss:.6f} pu")
